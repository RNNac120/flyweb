...
export default function CadastrarManutencao() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [mecanicosResponse] = await Promise.all([fetch("/api/usuarios")])

        const [avioesResponse] = await Promise.all([fetch("/api/avioes")])
      ...
      <div className="flex flex-col gap-4 p-8 text-black w-3/5 h-5/6 items-center rounded-lg border bg-slate-200 justify-center overflow-scroll">
        ...
        {loading ? (
          ...
          <form onSubmit={handleSubmit} className="w-full">
            <div className="mb-4">
              <label htmlFor="mecanico" className="block text-gray-600">
                Mecânico
              </label>
              <select
                id="mecanico"
            ...
            <div className="mb-4">
              <label htmlFor="aviao" className="block text-gray-600">
                Avião
              </label>
              <select
                id="aviao"
            ...
            <div className="mb-4">
              <label htmlFor="inicio_mnt" className="block text-gray-600">
            ...
            <div className="mb-4">
              <label htmlFor="fim_mnt" className="block text-gray-600">
                Fim da Manutenção
              </label>
              <input
                id="fim_mnt"
            ...
            <div className="mb-4">
              <label htmlFor="descricao" className="block text-gray-600">
