...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === "GET") {
    ...
    try {
      ...
      const { tipo_comp, nota, comentarios, aula } = req.body

      if (!tipo_comp || !nota || !comentarios || !aula) {
      ...
      const novoCompetencia = await prisma.competencia.create({
        data: {
          tipo_comp,
          nota: Number.parseInt(nota, 10),
          comentarios,
          aula: Number.parseInt(aula, 10),
        },
      })
    ...
    try {
      ...
      const competenciaAtualizado = await prisma.competencia.update({
        ...
        data: {
          ...
          aula: aula !== undefined ? Number.parseInt(aula, 10) : competenciaExistente.aula,
          tipo_comp: tipo_comp,
        },
      })
