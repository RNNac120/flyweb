...
export default function ListarAulas() {
    ...
    const aulasFiltradas = aulas.filter((aula) =>
        aula.modulo?.toLowerCase().includes(pesquisa.toLowerCase())
    );
                        ...
                        <tbody>
                            {aulasFiltradas.map((aula: any) => (
                                <tr key={aula.id_aula}>
                                    ...
                                    <td className="border p-2 text-center">
                                        {aula.aviao_usado || "Avião não encontrado"}
                                    </td>
                                    <td className="border p-2 text-center">
                                        {aula.modulo || "Módulo não encontrado"}
                                    </td>
                                    <td className="border p-2 text-center">
                                        {aula.aluno || "Aluno não encontrado"}
                                    </td>
                                    <td className="border p-2 text-center">
                                        {aula.instrutor || "Instrutor não encontrado"}
                                    </td>
